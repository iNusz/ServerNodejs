...
public class WriteActivity extends AppCompatActivity {
    ...
    private void postData(String title, String author, String content) {
        ...
        // RequestBody 에 미디어타입과 String으로 변환된 데이터를 담아서 전송
        RequestBody body = RequestBody.create(
                MediaType.parse("application/json"),   // 파싱을 하면된다 -> ("audio/mpeg")
                gson.toJson(bbs)
        );
        ...
        observable.subscribeOn(Schedulers.io())
                ...
                .subscribe(
                        responseBody -> {
                            String result = responseBody.string(); // 결과 코드를 넘겨서 처리...

                            /*
                            todo 1. 내가 정상적으로 새글을 입력한 후 데이터를 전송하고 종료
                            todo 2. 새글을 전송하지 않고 화면을 그냥 종료

                            todo 위의 두가지를 구분해서 결과값을 호출한 MainActivity로 넘겨서 처리
                             */
                            finish();
                        }
                );
